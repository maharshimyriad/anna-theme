...
$heading = anna_get_option( 'move_pg_cta_heading' );
$body    = anna_get_option( 'move_pg_cta_body' );

$buttons = array();
for ( $i = 1; $i <= 3; $i++ ) {
	$text = anna_get_option( 'move_pg_cta_btn_' . $i . '_text' );
	$url  = anna_get_option( 'move_pg_cta_btn_' . $i . '_url' );
	if ( $text && $url ) {
...
<section class="anna-section anna-section--md anna-move-cta">
	<div class="anna-container anna-container--narrow anna-text-center" data-reveal="fade-up">
		...
		<?php if ( ! empty( $buttons ) ) : ?>
			<div class="anna-move-cta__actions anna-flex anna-flex--center anna-flex--wrap anna-flex--gap-sm">
