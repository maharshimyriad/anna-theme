...
{
    register_setting("anna_theme_options_group", "anna_theme_options", [
        "type" => "array",
...
function anna_seed_about_page_post_content()
{
    ...
    $story_heading =
        $defaults["about_pg_story_heading"] ?? "My story the beginning";
    ...
    $coach_eyebrow =
        $defaults["about_pg_coach_eyebrow"] ?? "How I Became a Coach";
    ...
    $people_items = isset($defaults["about_pg_people_items_text"])
        ? preg_split('/\r\n|\r|\n/', $defaults["about_pg_people_items_text"])
        : [];
    $people_items = array_values(
        array_filter(array_map("trim", (array) $people_items)),
    );

    $connect_eyebrow =
        $defaults["about_pg_connect_eyebrow"] ??
        "If something in my story resonates";
    ...
    if ($hero_subheading) {
        $content .=
            "<p><strong>" . esc_html($hero_subheading) . "</strong></p>";
    }
    if ($hero_description) {
    ...
    if ($coach_button_text && $coach_button_url) {
        $content .=
            '<p><a href="' .
            esc_url($coach_button_url) .
            '">' .
            esc_html($coach_button_text) .
            "</a></p>";
    }
    ...
    if ($connect_button_text && $connect_button_url) {
        $content .=
            '<p><a href="' .
            esc_url($connect_button_url) .
            '">' .
            esc_html($connect_button_text) .
            "</a></p>";
    }
...
function anna_migrate_about_page_copy_20260528()
{
    ...
    $people_items_current = $options["about_pg_people_items"] ?? [];
    $people_text_current = isset($options["about_pg_people_items_text"])
        ? (string) $options["about_pg_people_items_text"]
        : "";

    $changed = false;
    if (!is_array($people_items_current) || empty($people_items_current)) {
        if (
            "" !== trim($people_text_current) &&
            function_exists("anna_parse_about_people_items")
        ) {
            $options["about_pg_people_items"] = anna_parse_about_people_items(
                $people_text_current,
            );
        } else {
            $options["about_pg_people_items"] =
                $defaults["about_pg_people_items"] ?? [];
        }
        $options["about_pg_people_heading"] =
            (string) ($defaults["about_pg_people_heading"] ?? "");
        $options["about_pg_people_body"] =
            (string) ($defaults["about_pg_people_body"] ?? "");
        $options["about_pg_people_eyebrow"] =
            (string) ($defaults["about_pg_people_eyebrow"] ?? "");
        $changed = true;
    }
...
function anna_get_default_options()
{
    $defaults = [
        ...
        "header_style" => "solid",
        "discovery_call_url" => ANNA_DISCOVERY_CALL_URL,
        "header_cta_text" => "Book a Discovery Call",
        "header_cta_url" => ANNA_DISCOVERY_CALL_URL,
        "hero_eyebrow" =>
            "Life Coach · motivational Speaker · Olympian · Melbourne",
        "hero_heading" => "You know what to do. <br>You're just not doing it.",
        "hero_description" =>
            "I help people understand why change has felt so hard and create the conditions for change that actually lasts. Not through willpower. Through understanding what has been running underneath.",
        "hero_trust_text" => "",
        "hero_image_id" => "",
        "intro_image_id" => "",
        ...
        "intro_quote" =>
            "If you don't know, you don't know. But once you do, everything becomes possible.",
        "intro_quote_cite" => "Anna Baylis",
        "intro_body" =>
            "<p>Most people who find me have already tried. They've done the therapy, read the books, set the goals. Something keeps getting in the way.</p><p>That something lives in the subconscious. In the programs and patterns formed long before you had any say in them. My work is to help you see those programs and then change them. Not through willpower. Through understanding.</p>",
        "recognition_eyebrow" => "",
        ...
        "about_body" =>
            "<p>I became the coach I am because of what I've lived through, not in spite of it. From two decades of elite sport to my own rock bottom and back, I understand real transformation from the inside out.</p><p>My approach is different to most coaches. I work bottom-up, through the body, through the subconscious, helping clients understand the programs that have been running underneath their thoughts, feelings and behaviour for years.</p>",
        ...
        "testimonials_summary" =>
            "5.0 · Google Reviews · Australia and worldwide",
        "testimonials_cta_text" => "Read all reviews",
        "testimonials_cta_url" => "#",
        "cta_eyebrow" => "Ready to begin?",
        "cta_heading" =>
            "By connecting mindset, emotional health and physical wellbeing",
        "cta_description" => "you can move towards your most fulfilling life.",
        "cta_trust" =>
            "Start with a complimentary discovery call. No obligation, just a conversation to see if we're the right fit.",
        "cta_primary_text" => "Book a Discovery Call",
        "cta_primary_url" => ANNA_DISCOVERY_CALL_URL,
        "cta_secondary_text" => "",
        "cta_secondary_url" => "#",
        "cta_image_id" => "",
        "footer_description" =>
            "Trauma-informed life coaching, speaking and community for people ready to create lasting change from the inside out.\n\nBased in Melbourne, Australia.",
        "contact_email" => "info@annabaylis.com.au",
        "contact_phone" => "0486 082 013",
        "contact_address" =>
            "In person - South East Melbourne\nOnline - Australia and worldwide",
        ...
        "about_pg_people_items" => [
            ...
            [
                ...
                "org" => "The Coaching Institute",
            ],
            [
                "logo_id" => 0,
                "initials" => "NLP",
                "title" => "NLP Practitioner and Master Practitioner",
                "org" => "Institute of Empowered Psychology",
            ],
            [
                "logo_id" => 0,
                "initials" => "HY",
            ...
            [
                "logo_id" => 0,
                "initials" => "IFS",
                "title" => "Parts work — Internal Family Systems informed",
                "org" => "Embodied Philosophy Western School",
            ],
            [
                "logo_id" => 0,
                "initials" => "CI",
                "title" => "Masters — currently completing",
                "org" => "Gabor Maté",
            ],
            [
                "logo_id" => 0,
                "initials" => "CT",
                "title" => "102 five-star Google reviews",
                "org" => "Anodea Judith",
            ],
            [
                "logo_id" => 0,
                "initials" => "NR",
                "title" => "Honours — Food Science and Nutrition",
                "org" => "Deakin University",
            ],
            [
                "logo_id" => 0,
                "initials" => "EI",
                "title" => "Emotional Intimacy Coach",
                "org" => "The Coaching Institute",
            ],
            [
                "logo_id" => 0,
            ...
            [
                ...
                "org" => "The Centre for Healing",
            ],
            [
                "logo_id" => 0,
                "initials" => "SP",
                "title" => "Personal trainer — 7+ years",
                "org" => "NeuroAffective Touch Institute",
            ],
        ],
        ...
        "about_pg_connect_button_url" => ANNA_DISCOVERY_CALL_URL,

        // Coaching page (template: page-coaching.php).
        "coaching_pg_hero_eyebrow" =>
            "1-1 Life Coaching · Melbourne and Online",
        "coaching_pg_hero_heading" => "Real change.\nFrom the inside out.",
        "coaching_pg_hero_description" =>
            "For the person who is ready to understand what has actually been getting in the way.",
        "coaching_pg_hero_tags_text" => "",
        "coaching_pg_hero_image_id" => "",
        "coaching_pg_hero_button_text" => "Book a Discovery Call",
        "coaching_pg_hero_button_url" => ANNA_DISCOVERY_CALL_URL,
        "coaching_pg_what_eyebrow" => "What this is",
        "coaching_pg_what_heading" => "Different to most talk therapies.",
        "coaching_pg_what_body" =>
            "My approach is unlike most talk therapies. I work bottom-up — through the body and the nervous system — to access the subconscious and get to the root of what is actually running underneath your behaviour.\n\nThe framework I use with every client is Awareness, Acceptance and Action. We start by seeing clearly what is happening in your thoughts, your body, and your patterns. We move through honest acknowledgment of what is here. Then we take aligned action — not forced, not pushed, but rooted.\n\nI draw on parts work, somatic practice, NLP, Timeline Therapy and trauma-informed tools. Because lasting change is not just cognitive — it lives in the body.\n\nThis is coaching for people who are ready to go deeper than surface-level goals.",
        "coaching_pg_what_button_text" => "Book a Discovery Call",
        "coaching_pg_what_button_url" => ANNA_DISCOVERY_CALL_URL,
        "coaching_pg_what_card_heading" => "Does this sound like you?",
        "coaching_pg_what_card_items" => [
            ["text" => "You know what you need to do but you're not doing it"],
            [
                "text" =>
                    "You've tried therapy, programs and self-help and something still feels missing",
            ],
            ["text" => "You put everyone else first and run on empty"],
            [
                "text" =>
                    "You sense there's more available to you but don't know how to access it",
            ],
            ["text" => "You want to feel genuinely well, not just functional"],
        ],
        "coaching_pg_pillars_eyebrow" => "How I Work",
        "coaching_pg_pillars_heading" => "Three pillars of lasting change.",
        "coaching_pg_pillar_items" => [
            [
                "number" => "01",
                "title" => "Mindset and Subconscious",
                "body" =>
                    "We unpack your beliefs, values and the emotional states running underneath your behaviour. Using NLP, Timeline Therapy and hypnotherapy to access and shift the programs that have been driving your reality — often since childhood.",
            ],
            [
                "number" => "02",
                "title" => "Emotional Healing and Parts Work",
                "body" =>
                    "Using Internal Family Systems, Inner Child work, trauma-informed practice and Compassionate Inquiry to meet the parts of you that have been protecting old wounds — with curiosity and compassion rather than judgment.",
            ],
            [
                "number" => "03",
                "title" => "Body, Movement and Nutrition",
                "body" =>
                    "Drawing on my degree in Human Movement and Honours in Nutrition, we look at the whole person — how you move, how you nourish yourself, and how your physical health both reflects and affects your inner world.",
            ],
        ],
        ...
        "coaching_pg_work_topics_items" => [
            [
                "text" =>
                    "The subconscious programs and beliefs shaping your current reality",
            ],
            [
                "text" =>
                    "The parts of you that protect, sabotage or hold you back — and why they're there",
            ],
            [
                "text" =>
                    "Emotions and memories from the past that are still running in the present",
            ],
            [
                "text" =>
                    "Your values, identity and what actually matters to you",
            ],
            [
                "text" =>
                    "Nutrition and lifestyle as part of a whole-person approach",
            ],
            [
                "text" =>
                    "Practical strategies and tools you can use in everyday life",
            ],
            [
                "text" =>
                    "How to go from reacting to responding — above the line thinking",
            ],
            ["text" => "The nervous system — stress, regulation, safety"],
        ],
        "coaching_pg_work_gains_items" => [
            [
                "text" =>
                    "A deep **understanding** of themselves and the patterns driving their behaviour",
            ],
            [
                "text" =>
                    "**Freedom** from beliefs and identities that have been holding them back",
            ],
            [
                "text" =>
                    "Greater emotional **clarity**, **balance** and self-awareness",
            ],
            [
                "text" =>
                    "Practical **tools** for navigating stress, challenge and change",
            ],
            [
                "text" =>
                    "Renewed sense of **purpose**, **direction** and **confidence**",
            ],
            [
                "text" =>
                    "Improved relationships, **communication** and **connection**",
            ],
            [
                "text" =>
                    "A genuine felt sense of **possibility** — not just intellectually, but in the body",
            ],
        ],
        ...
        "coaching_pg_expect_body" =>
            "We begin with a complimentary discovery call to explore whether we're the right fit. There is no pressure and no obligation — just a conversation.\n\nI work with men, women and couples across Australia. In person in South East Melbourne and online via Zoom. I also offer guided outdoor Walk and Talk sessions for those who work best in nature.",
        "coaching_pg_expect_quote" =>
        ...
        "coaching_pg_expect_button_url" => ANNA_DISCOVERY_CALL_URL,
        "coaching_pg_expect_info_cards" => [
            [
                "label" => "Who I Work With",
                "body" =>
                    "Men, women and couples · All walks of life · Australia and worldwide · Teenagers by arrangement",
            ],
            [
                "label" => "Important Note",
                "body" =>
                    "Life coaching is not a replacement for medically advised counselling or psychiatric intervention. I work alongside your existing support where relevant.",
            ],
        ],
        "coaching_pg_faq_heading" => "Everything you need to know.",
        "coaching_pg_faq_items" => [
            [
                "question" =>
                    "What is the difference between life coaching and therapy?",
                "answer" =>
                    "While therapy often focuses on diagnosing and treating mental health conditions or deeply exploring the past, my coaching approach is forward-focused while still acknowledging and healing the root causes of current blocks. We use somatic and subconscious techniques to create tangible change in the present, rather than just talking about the problem.",
            ],
            [
                "question" => "Who is this type of coaching for?",
                "answer" =>
                    "Anyone who feels stuck, disconnected, or ready for deeper change — especially if you have tried talk therapy or self-help and something still feels missing.",
            ],
            [
                "question" =>
                    "How long are the sessions and how often do we meet?",
                "answer" =>
                    "Sessions are typically 60–90 minutes. Most clients meet weekly or fortnightly, depending on what you need.",
            ],
            [
                "question" =>
                    "Do I have to commit to a certain number of sessions?",
                "answer" =>
                    "No long-term lock-in is required. We agree on a rhythm that supports your goals and adjust as you progress.",
            ],
            [
                "question" => "Do you offer online sessions?",
                "answer" =>
                    "Yes — online sessions via Zoom are available across Australia and internationally.",
            ],
            [
                "question" => "What happens in a Walk and Talk session?",
                "answer" =>
                    "We meet outdoors for a guided walk while we talk. Many clients find movement helps them think more clearly and feel more grounded.",
            ],
            [
                "question" => "What should I expect in our first session?",
                "answer" =>
                    "We clarify what you want to change, explore what has been getting in the way, and outline a practical path forward tailored to you.",
            ],
            [
                "question" => "What is your cancellation policy?",
                "answer" =>
                    "Please provide at least 24 hours notice to reschedule. Late cancellations may be charged at the session rate.",
            ],
        ],
    ];
    ...
    if (function_exists("anna_get_oasis_theme_option_defaults")) {
        $defaults = array_merge(
            $defaults,
            anna_get_oasis_theme_option_defaults(),
        );
    }
    ...
    if (function_exists("anna_get_speaking_theme_option_defaults")) {
        $defaults = array_merge(
            $defaults,
            anna_get_speaking_theme_option_defaults(),
        );
    }
    ...
    if (function_exists("anna_get_mhs_theme_option_defaults")) {
        $defaults = array_merge(
            $defaults,
            anna_get_mhs_theme_option_defaults(),
        );
    }
    ...
    if (function_exists("anna_get_move_theme_option_defaults")) {
        $defaults = array_merge(
            $defaults,
            anna_get_move_theme_option_defaults(),
        );
    }
...
function anna_seed_oasis_page_defaults()
{
    ...
    foreach ($oasis_keys as $key) {
        $has_value = false;
        if (isset($options[$key])) {
            $has_value = is_array($options[$key])
                ? !empty($options[$key])
