...
function anna_render_speaking_page_settings_fields()
{
    ...
    anna_field_textarea(
        "speaking_pg_hero_heading",
        __("Heading", "anna-baylis"),
        __("One line per row.", "anna-baylis"),
        3,
    );
    anna_field_textarea(
    ...
    anna_field_media(
        "speaking_pg_hero_image_id",
        __("Background Image", "anna-baylis"),
    );
    anna_field_text(
        "speaking_pg_hero_button_text",
    ...
    anna_field_text(
        "speaking_pg_hero_secondary_text",
    ...
    anna_field_text(
        "speaking_pg_bring_heading_line1",
        __("Heading Line 1", "anna-baylis"),
    );
    anna_field_text(
        "speaking_pg_bring_heading_line2",
    ...
    anna_field_text(
        "speaking_pg_formats_eyebrow",
        __("Eyebrow", "anna-baylis"),
    );
    anna_field_text(
        "speaking_pg_formats_heading",
        __("Heading", "anna-baylis"),
    );
    anna_render_speaking_format_repeater();
    anna_field_text(
        "speaking_pg_formats_audience_heading",
        __("Audience Heading", "anna-baylis"),
    );
    anna_render_speaking_text_repeater(
    ...
    anna_field_text(
        "speaking_pg_takeaway_eyebrow",
        __("Eyebrow", "anna-baylis"),
    );
    anna_field_text(
        "speaking_pg_takeaway_heading",
        __("Heading", "anna-baylis"),
    );
    anna_field_textarea(
    ...
    anna_render_speaking_text_repeater(
        ...
        "speaking-takeaway",
        __("Use **word** for emphasis.", "anna-baylis"),
    );

    anna_field_heading(__("Book Anna to Speak", "anna-baylis"));
    anna_field_text("speaking_pg_book_eyebrow", __("Eyebrow", "anna-baylis"));
    anna_field_text(
        "speaking_pg_book_heading_line1",
        __("Heading Line 1", "anna-baylis"),
    );
    anna_field_text(
        "speaking_pg_book_heading_line2",
    ...
    anna_field_text(
        "speaking_pg_book_card_heading",
        __("Card Heading", "anna-baylis"),
    );
    anna_field_textarea(
        "speaking_pg_book_card_body",
        __("Card Body", "anna-baylis"),
        "",
        3,
    );
    anna_field_text(
        "speaking_pg_book_card_button_text",
    ...
    anna_field_text(
        "speaking_pg_book_card_contact_prefix",
        __("Contact Prefix Text", "anna-baylis"),
        __('e.g. "Or email me directly at"', "anna-baylis"),
    );
    anna_field_text(
        "speaking_pg_book_card_email",
        __("Contact Email", "anna-baylis"),
        __("Displayed as a mailto link.", "anna-baylis"),
        "email",
    );
    anna_field_text(
        "speaking_pg_book_card_footer",
        __("Card Footer Text", "anna-baylis"),
    );

    anna_field_heading(__("Recent Experience", "anna-baylis"));
    anna_field_text(
        "speaking_pg_experience_eyebrow",
        __("Eyebrow", "anna-baylis"),
    );
    anna_field_text(
        "speaking_pg_experience_heading_primary",
        __("Heading (Primary)", "anna-baylis"),
    );
    anna_field_text(
        "speaking_pg_experience_heading_secondary",
    ...
    anna_field_text(
        "speaking_pg_experience_link_prefix",
        __("Link Prefix", "anna-baylis"),
    );
    anna_field_text(
        "speaking_pg_experience_link_label",
    ...
    anna_field_text(
        "speaking_pg_experience_testimonial_name",
        __("Testimonial Name", "anna-baylis"),
    );
    anna_field_text(
        "speaking_pg_experience_testimonial_role",
        __("Testimonial Role", "anna-baylis"),
    );
}

function anna_render_speaking_text_repeater(
    $option_key,
    $label,
    $slug,
    $desc = "",
) {
    $items = anna_get_option(
        $option_key,
        anna_get_speaking_default_content()[
            str_replace("speaking_pg_", "", $option_key)
        ] ?? [],
    );
	...
	<tr>
		...
		<td>
			<?php if ($desc): ?><p class="description"><?php echo esc_html(
    $desc,
); ?></p><?php endif; ?>
			<div class="anna-admin-repeater" data-anna-repeater="<?php echo esc_attr(
       $slug,
   ); ?>">
				<div class="anna-admin-repeater__rows" data-anna-repeater-rows="true">
					<?php foreach ($items as $i => $item): ?>
						<div class="anna-admin-repeater__row" data-anna-repeater-row="true">
							<input type="text" class="large-text" name="anna_theme_options[<?php echo esc_attr(
           $option_key,
       ); ?>][<?php echo esc_attr($i); ?>][text]" value="<?php echo esc_attr(
    $item["text"] ?? "",
); ?>">
...
function anna_render_speaking_stat_repeater()
{
    ...
    $items = anna_normalize_speaking_stat_items(
        anna_get_option(
            $option_key,
            anna_get_speaking_default_content()["hero_stat_items"] ?? [],
        ),
    );
    ?>
	<tr>
		...
		<td>
			<div class="anna-admin-repeater" data-anna-repeater="speaking-stats">
				<div class="anna-admin-repeater__rows" data-anna-repeater-rows="true">
					<?php foreach ($items as $i => $item): ?>
						<div class="anna-admin-repeater__row" data-anna-repeater-row="true">
							<p><input type="text" class="small-text" name="anna_theme_options[<?php echo esc_attr(
           $option_key,
       ); ?>][<?php echo esc_attr($i); ?>][value]" value="<?php echo esc_attr(
    $item["value"] ?? "",
); ?>" placeholder="<?php esc_attr_e("Value", "anna-baylis"); ?>"></p>
							<p><input type="text" class="large-text" name="anna_theme_options[<?php echo esc_attr(
           $option_key,
       ); ?>][<?php echo esc_attr($i); ?>][label]" value="<?php echo esc_attr(
    $item["label"] ?? "",
); ?>" placeholder="<?php esc_attr_e("Label", "anna-baylis"); ?>"></p>
							<button type="button" class="button-link-delete anna-admin-repeater__remove" data-anna-repeater-remove="true"><?php esc_html_e(
           "Remove",
           "anna-baylis",
       ); ?></button><hr>
        ...
        "anna-baylis",
				...
				<template data-anna-repeater-template="true">
					<div class="anna-admin-repeater__row" data-anna-repeater-row="true">
						<p><input type="text" class="small-text" name="anna_theme_options[<?php echo esc_attr(
          $option_key,
      ); ?>][__INDEX__][value]" value=""></p>
						<p><input type="text" class="large-text" name="anna_theme_options[<?php echo esc_attr(
          $option_key,
      ); ?>][__INDEX__][label]" value=""></p>
						<button type="button" class="button-link-delete anna-admin-repeater__remove" data-anna-repeater-remove="true"><?php esc_html_e(
          "Remove",
          "anna-baylis",
      ); ?></button><hr>
...
function anna_render_speaking_topic_repeater()
{
    ...
    $items = anna_normalize_speaking_topic_cards(
        anna_get_option(
            $option_key,
            anna_get_speaking_default_content()["topics_card_items"] ?? [],
        ),
    );
    ?>
	<tr>
		...
		<td>
           ...
           "Icon",
           "anna-baylis",
       ); ?> <input type="text" class="small-text" name="anna_theme_options[<?php echo esc_attr(
     $option_key,
 ); ?>][<?php echo esc_attr($i); ?>][icon]" value="<?php echo esc_attr(
    $item["icon"] ?? "brain",
); ?>"></label></p>
							<p><input type="text" class="large-text" name="anna_theme_options[<?php echo esc_attr(
           $option_key,
       ); ?>][<?php echo esc_attr($i); ?>][title]" value="<?php echo esc_attr(
    $item["title"] ?? "",
); ?>"></p>
							<p><textarea class="large-text" rows="3" name="anna_theme_options[<?php echo esc_attr(
           $option_key,
       ); ?>][<?php echo esc_attr($i); ?>][body]"><?php echo esc_textarea(
    $item["body"] ?? "",
); ?></textarea></p>
							<button type="button" class="button-link-delete anna-admin-repeater__remove" data-anna-repeater-remove="true"><?php esc_html_e(
           "Remove",
        ...
        "anna-baylis",
				...
				<template data-anna-repeater-template="true">
					<div class="anna-admin-repeater__row" data-anna-repeater-row="true">
						<p><input type="text" class="small-text" name="anna_theme_options[<?php echo esc_attr(
          $option_key,
      ); ?>][__INDEX__][icon]" value="brain"></p>
						<p><input type="text" class="large-text" name="anna_theme_options[<?php echo esc_attr(
          $option_key,
      ); ?>][__INDEX__][title]" value=""></p>
						<p><textarea class="large-text" rows="3" name="anna_theme_options[<?php echo esc_attr(
          $option_key,
      ); ?>][__INDEX__][body]"></textarea></p>
						<button type="button" class="button-link-delete anna-admin-repeater__remove" data-anna-repeater-remove="true"><?php esc_html_e(
          "Remove",
          "anna-baylis",
      ); ?></button><hr>
...
function anna_render_speaking_format_repeater()
{
    ...
    $items = anna_normalize_speaking_format_cards(
        anna_get_option(
            $option_key,
            anna_get_speaking_default_content()["formats_card_items"] ?? [],
        ),
    );
    ?>
	<tr>
				...
				<div class="anna-admin-repeater__rows" data-anna-repeater-rows="true">
					<?php foreach ($items as $i => $item): ?>
						<div class="anna-admin-repeater__row" data-anna-repeater-row="true">
							<p><input type="text" class="small-text" name="anna_theme_options[<?php echo esc_attr(
           $option_key,
       ); ?>][<?php echo esc_attr($i); ?>][number]" value="<?php echo esc_attr(
    $item["number"] ?? "",
); ?>" placeholder="01"></p>
							<p><input type="text" class="large-text" name="anna_theme_options[<?php echo esc_attr(
           $option_key,
