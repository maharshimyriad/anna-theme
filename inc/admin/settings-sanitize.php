...
function anna_get_tab_fields_map()
{
    $tab_map = [
        ...
        "oasis_page" => array_keys(
            function_exists("anna_get_oasis_theme_option_defaults")
                ? anna_get_oasis_theme_option_defaults()
                : [],
        ),
        "speaking_page" => array_keys(
            function_exists("anna_get_speaking_theme_option_defaults")
                ? anna_get_speaking_theme_option_defaults()
                : [],
        ),
        "mhs_page" => array_keys(
            function_exists("anna_get_mhs_theme_option_defaults")
                ? anna_get_mhs_theme_option_defaults()
                : [],
        ),
        "move_page" => array_keys(
            function_exists("anna_get_move_theme_option_defaults")
        ...
        "coaching_page" => [
            ...
            "coaching_pg_hero_button_text",
            "coaching_pg_hero_button_url",
            "coaching_pg_what_eyebrow",
            "coaching_pg_what_heading",
            "coaching_pg_what_body",
            "coaching_pg_what_button_text",
            "coaching_pg_what_button_url",
            "coaching_pg_what_card_heading",
            "coaching_pg_what_card_items",
            "coaching_pg_pillars_eyebrow",
            "coaching_pg_pillars_heading",
            "coaching_pg_pillar_items",
            "coaching_pg_work_eyebrow",
            "coaching_pg_work_heading",
            "coaching_pg_work_gains_heading",
            "coaching_pg_work_topics_items",
            "coaching_pg_work_gains_items",
            "coaching_pg_expect_eyebrow",
            "coaching_pg_expect_heading_line1",
            "coaching_pg_expect_heading_line2",
            "coaching_pg_expect_body",
            "coaching_pg_expect_quote",
            "coaching_pg_expect_button_text",
            "coaching_pg_expect_button_url",
            "coaching_pg_expect_info_cards",
            "coaching_pg_faq_heading",
        ...
        "social" => ["social_links"],
        "animations" => ["animations_enabled", "animation_speed"],
        "seo" => [
...
function anna_sanitize_single_option($key, $value)
{
    ...
    $url_fields = [
        ...
        "coaching_pg_what_button_url",
        "coaching_pg_expect_button_url",
        "oasis_pg_hero_button_url",
        "oasis_pg_what_footer_url",
        "oasis_pg_begun_link_url",
        "oasis_pg_waitlist_button_url",
    ...
    $int_fields = [
        ...
        "about_pg_coach_image_id",
        "coaching_pg_hero_image_id",
        "oasis_pg_hero_image_id",
        "oasis_pg_begun_image_id",
        "speaking_pg_hero_image_id",
        "speaking_pg_bring_image_id",
    ];
    ...
    if ("about_pg_people_items" === $key) {
        ...
        foreach ($value as $row) {
            ...
            $org = sanitize_textarea_field(
                $row["description"] ?? ($row["org"] ?? ""),
            );
    ...
    if (
        ...
        return function_exists("anna_normalize_coaching_text_items")
            ? anna_normalize_coaching_text_items($value)
            : [];
    }

    if ("coaching_pg_pillar_items" === $key) {
        return function_exists("anna_normalize_coaching_pillar_items")
            ? anna_normalize_coaching_pillar_items($value)
            : [];
    }

    if ("coaching_pg_expect_info_cards" === $key) {
        return function_exists("anna_normalize_coaching_info_cards")
            ? anna_normalize_coaching_info_cards($value)
            : [];
    }

    if ("coaching_pg_faq_items" === $key) {
        return function_exists("anna_normalize_coaching_faq_items")
            ? anna_normalize_coaching_faq_items($value)
            : [];
    }

    if (
        str_starts_with($key, "oasis_pg_") &&
        function_exists("anna_sanitize_oasis_option")
    ) {
        return anna_sanitize_oasis_option($key, $value);
    }

    if (
        str_starts_with($key, "speaking_pg_") &&
        function_exists("anna_sanitize_speaking_option")
    ) {
        return anna_sanitize_speaking_option($key, $value);
    }

    if (
        str_starts_with($key, "mhs_pg_") &&
        function_exists("anna_sanitize_mhs_option")
    ) {
        return anna_sanitize_mhs_option($key, $value);
    }

    if (
        str_starts_with($key, "move_pg_") &&
        function_exists("anna_sanitize_move_option")
    ) {
...
{
    $defaults = anna_get_default_options();
    $existing = get_option("anna_theme_options", []);
    $existing = is_array($existing) ? $existing : [];
    $tab_fields_map = anna_get_tab_fields_map();
    $active_tab = isset($input["_anna_active_tab"])
        ? sanitize_key($input["_anna_active_tab"])
        : "";
    ...
    if ($active_tab && isset($tab_fields_map[$active_tab])) {
        foreach ($tab_fields_map[$active_tab] as $key) {
            if (isset($input[$key])) {
                $sanitized[$key] = anna_sanitize_single_option(
                    $key,
                    $input[$key],
                );
