...
function anna_get_contact_default_content() {
	return array(
		...
		'cta_card_body'        => "Start with a complimentary discovery call to see if we're the right fit.",
		...
		'form_response_note'   => 'I typically respond within 24–48 hours. For urgent matters, please call directly.',
		'form_action_url'      => '',
	);
}
...
function anna_get_contact_page_content() {
	...
	foreach ( $defaults as $key => $default_value ) {
		if ( 'hero_image_id' === $key ) {
			$content[ $key ] = isset( $saved[ $key ] ) ? absint( $saved[ $key ] ) : $default_value;
			continue;
		}
...
function anna_handle_contact_form() {
	...
	if ( ! isset( $_POST['anna_contact_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['anna_contact_nonce'] ) ), 'anna_contact_form' ) ) {
